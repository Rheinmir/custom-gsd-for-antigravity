...
Template for `.planning/phases/XX-name/{phase}-USER-SETUP.md` - human-required configuration that the AI cannot automate.

**Purpose:** Document setup tasks that literally require human action - account creation, dashboard configuration, secret retrieval. The system automates everything possible; this file captures only what remains.
...
Complete these items for the integration to function. The system automated everything possible; these items require human access to external dashboards/accounts.
...
**USER-SETUP.md contains ONLY what the AI literally cannot do.**

| AI CAN Do (not in USER-SETUP) | AI CANNOT Do (→ USER-SETUP) |
|-----------------------------------|--------------------------------|
...
**The test:** "Does this require a human in a browser, accessing an account the AI doesn't have credentials for?"
- Yes → USER-SETUP.md
- No → The AI does it automatically
...
**Never include:** Actual secret values. Steps the AI can automate (package installs, code changes).
