...
const hasAntigravity = args.includes("--antigravity"); // Antigravity support
 ...
 * Wraps the prompt in a workflow structure
 */
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (isOpencode) {
        ...
        // Antigravity: Convert to Workflow format
        let content = fs.readFileSync(srcPath, "utf8");
        const claudeDirRegex = /~\/\.claude\//g;
        // Replace paths with local .agent paths or global paths
        ...
        if (destDir.includes(".gemini/antigravity")) {
          ...
          // 2. Base path -> ~/.gemini/antigravity/
        ...
        // Rename plan-phase.md to gsd-plan-phase.md for clarity in workflows list
...
function uninstall(isGlobal, runtime = "claude") {
  ...
  if (runtime === "antigravity") {
    // Antigravity: remove workflows/gsd-*.md and skills/gsd
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        if (modified) {
          ...
          removedCount++;
          console.log(
            `  ${green}✓${reset} Removed GSD permissions from opencode.json`,
          );
        }
      } catch (e) {
...
function configureOpencodePermissions() {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      console.log(
        `  ${yellow}⚠${reset} opencode.json had invalid JSON, recreating`,
      );
    }
  }
...
function install(isGlobal, runtime = "claude") {
  ...
  // Path prefix: Antigravity uses .agent/ always relative to workspace root basically
  // But inside files we might reference .agent/skills/...
  ...
  if (isOpencode) {
    ...
    // Antigravity: Install commands as workflows
    const workflowsDir = path.join(targetDir, "workflows");
    fs.mkdirSync(workflowsDir, { recursive: true });

    // Commands source
    const gsdSrc = path.join(src, "commands", "gsd");
    // We pass workflowsDir as dest, but copyWithPathReplacement for 'antigravity'
    // will act specially (flattening and renaming) inside the loop for files
    copyWithPathReplacement(gsdSrc, workflowsDir, pathPrefix, runtime);
    if (verifyInstalled(workflowsDir, "workflows/gsd-*")) {
      console.log(`  ${green}✓${reset} Installed workflows/gsd-*`);
    } else {
  ...
  let settingsPath, settings, statuslineCommand;

  if (fs.existsSync(agentsSrc)) {
    // Copy agents to agents directory (or skills/gsd for Antigravity)
    const agentsSrc = path.join(src, "agents");
    if (fs.existsSync(agentsSrc)) {
      const agentsDest =
        runtime === "antigravity"
    ...
    settings = cleanupOrphanedHooks(readSettings(settingsPath));
...
function finishInstall(
  ...
  const command =
    isOpencode || runtime === "antigravity" ? "gsd-help" : "/gsd:help";
  console.log(`
...
function promptRuntime(callback) {
  ...
  ${cyan}4${reset}) Antigravity ${dim}(.agent)${reset}
  ${cyan}5${reset}) All
`);
  ...
  rl.question(`  Choice ${dim}[1]${reset}: `, (answer) => {
    ...
    const choice = answer.trim() || "1";
    if (choice === "5") {
      callback(["claude", "opencode", "gemini", "antigravity"]);
    } else if (choice === "4") {
...
function installAllRuntimes(runtimes, isGlobal, isInteractive) {
  ...
  const antigravityResult = results.find((r) => r.runtime === "antigravity");
  const opencodeResult = results.find((r) => r.runtime === "opencode");

  if (claudeResult || geminiResult || antigravityResult) {
    // Use whichever settings exist to check for existing statusline
    const primaryResult = claudeResult || geminiResult || antigravityResult;

    handleStatusline(
      primaryResult.settings,
      isInteractive,
      (shouldInstallStatusline) => {
        if (claudeResult) {
          finishInstall(
        ...
        if (antigravityResult) {
          finishInstall(
            antigravityResult.settingsPath,
            antigravityResult.settings,
            antigravityResult.statuslineCommand,
            shouldInstallStatusline,
            "antigravity",
          );
        }
